// letstry1-report.js — the module-contract adapter, same role as
// eiken-report.js / nh6-report.js.
// Replaces Let's Try 1's own login modal + independent Supabase backend (a
...
//
// index.html's saveResultToSupabase() (unchanged in spirit from the
// original app, just stripped of its currentUser gate) calls
// window.hk.syncQuizResult() directly — so instead of wrapping something
// that already exists, this DEFINES window.hk fresh, implementing only
// the one method still called now that the login-modal code (the only
// caller of every other window.hk method) has been removed.
(async function () {
  ...
  // ── Account bubble (name + sign-out) — identical pattern to eiken-report.js / nh6-report.js ──
  ...
  const [{ data: moduleRow }, { data: enrollmentRows }] = await Promise.all([
    sb.from('modules').select('id').eq('key', 'letstry1').maybeSingle(),
  ...
  if (!moduleId || !schoolId) {
    console.warn('[LT1Report] no module/school context for this account — results will not be reported.');
  }

  // The shim itself. index.html's saveResultToSupabase() calls exactly
  // this one method, guarded by `!window.hk`:
  //   await window.hk.syncQuizResult({ app_id, level, setId, category, correct, total });
  // NOTE: unlike NH6 (where level and setId differ, e.g. level='u1',
  // setId='grammar'), this app's saveResultToSupabase() always sets
  // level === setId === 'unit'+unitId. Using `${level}/${setId}` here
  // (as NH6's shim does) would produce a redundant ref like
  // "letstry1/unit0/unit0/<ts>". Use `category` (the actType: quiz/
