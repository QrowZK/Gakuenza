...
// rest — matching the sansu3/kokugo3 pattern. Foregrounding is async and
// non-blocking; if the fetch fails or returns null, all units show normally.
(function () {
  'use strict';

  const { STRANDS } = window.RIKA4_DATA;
  ...
  function finishDrill() {
    ...
    if (window.Rika4Report) {
      window.Rika4Report.report({
        unitKey: u.key,
  ...
  async function initAccount() {
    if (!window.Rika4Report) return;
    try {
      const profile = await window.Rika4Report.getProfile();
      if (!profile) return;
      ...
      $('signoutBtn').addEventListener('click', async () => {
        await window.Rika4Report.signOut();
        window.location.href = '../../hub/login.html';
      });
    } catch (e) {
      console.log('[Rika4] account bubble skipped:', e && e.message);
    }
  }
  ...
  function initFocusUnits() {
    if (!window.Rika4Report || !window.Rika4Report.getFocusUnits) return;
    window.Rika4Report.getFocusUnits().then(function (keys) {
      if (keys && keys.length) { focusUnits = new Set(keys); renderMenu(); }
    }).catch(function () { /* fail soft — all units stay visible */ });
  }

  // expose internals for tests
  window.__rika4Test = { state, UNIT_BY_KEY, buildQuestions, shuffle,
