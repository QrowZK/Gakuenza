// rika4-report.js — backend wiring for the 理科4年 module.
...
//
// rika4 also honors class_modules.focus_units via getFocusUnits(), matching
// the established sansu3/kokugo3 pattern: return the union of focus-unit keys
// across the student's classes, or null ("all units") when the scope is
// ambiguous. Foregrounds, never hides; fails soft to null on any error.
(function () {
  'use strict';

  const MODULE_KEY = 'rika4';
  let client = null;
  let ctxPromise = null;
  ...
  function getClient() {
    ...
    try {
      ...
      console.log('[Rika4Report] supabase client unavailable:', e && e.message);
    }
    return client;
  }
  ...
  async function resolveContext() {
    ...
    if (!session || !session.user) {
      console.log('[Rika4Report] no session — skipping reporting.');
      return null;
    }
    ...
    if (!mod || !schoolId) {
      console.log('[Rika4Report] no module/school context — skipping reporting.');
  ...
  function getContext() {
    if (!ctxPromise) ctxPromise = resolveContext().catch(function (e) {
      console.log('[Rika4Report] context resolution failed:', e && e.message);
      return null;
    });
    return ctxPromise;
  }
  ...
  async function report(result) {
    try {
      ...
      if (!window.HubCommon || !window.HubCommon.reportActivityWithItems) {
        console.log('[Rika4Report] HubCommon.reportActivityWithItems unavailable — skipping.');
        return false;
      }
      ...
      // activity_ref, so 'rika4/<unitKey>/<ts>' groups every attempt of the
      ...
      if (!out.ok) {
        console.log('[Rika4Report] insert failed:', out.error && out.error.message);
        return false;
      }
      return true;
    } catch (e) {
      console.log('[Rika4Report] report failed:', e && e.message);
      return false;
    }
  }
  ...
  // error resolves to null (no regression). Keys are rika4's unit keys
  // (u01_warm_season..u12_year_review); see hub/module-units.js.
  async function getFocusUnits() {
    try {
      ...
      console.log('[Rika4Report] focus-units resolution failed:', e && e.message);
      return null;
    }
  }
  ...
  window.Rika4Report = { report, getProfile, getFocusUnits, signOut, MODULE_KEY };
})();
