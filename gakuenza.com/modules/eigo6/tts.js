// tts.js — Reusable text-to-speech helper for NH6 Practice
// Uses the browser's built-in Web Speech API (no server, no API key needed).
'use strict';

const NH6TTS = (function () {
  ...
  function pickVoice() {
    ...
    // Prefer en-GB (matches NH6 textbook pronunciation conventions),
    // fall back to any English voice, then to whatever is available.
    return (
  ...
  function speak(text, opts) {
    ...
    utt.lang = 'en-GB';
    utt.rate = typeof opts.rate === 'number' ? opts.rate : 0.8; // a bit slower for beginners
    utt.pitch = 1.0;
...
//   <button class="tts-btn" data-tts="Text to read aloud" aria-label="発音を聞く">🔊</button>
document.addEventListener('click', function (e) {
  const btn = e.target.closest('.tts-btn');
  if (!btn) return;
  e.preventDefault();
  e.stopPropagation(); // don't let the click bubble into answer-choice handlers etc.

  const text = btn.getAttribute('data-tts');
  if (!text) return;

  if (!NH6TTS.supported()) {
    console.warn('[NH6TTS] Speech synthesis not supported in this browser.');
    return;
  }
