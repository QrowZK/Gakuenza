// 理科6年 module — app logic.
// Screens: menu (two strand families) -> lesson (optional) -> drill
// (choice + order questions) -> review (+ best-effort gradebook report).
//
// Adds unit-scoped pacing (class_modules.focus_units): a class's focus units
// are foregrounded on the menu (a "今週" badge + highlight) without hiding the
// rest — matching the sansu3/kokugo3/rika4 pattern. Foregrounding is async and
// non-blocking; if the fetch fails or returns null, all units show normally.
(function () {
  'use strict';

  const { STRANDS } = window.RIKA6_DATA;
  const GEN = window.RIKA6_GEN;
  ...
  // null = show all units normally; Set = foreground these keys.
  let focusUnits = null;
  ...
  function finishDrill() {
    ...
    if (window.Rika6Report) {
      window.Rika6Report.report({
        unitKey: u.key,
  ...
  async function initAccount() {
    if (!window.Rika6Report) return;
    try {
      const profile = await window.Rika6Report.getProfile();
      if (!profile) return;
      ...
      $('signoutBtn').addEventListener('click', async () => {
        await window.Rika6Report.signOut();
        window.location.href = '../../hub/login.html';
      });
    } catch (e) {
      console.log('[Rika6] account bubble skipped:', e && e.message);
    }
  }

  // ---------- unit-scoped pacing (focus_units) ----------
  // Async, non-blocking: the menu is already usable with all units; if the
  // fetch fails or returns null, nothing changes (no regression).
  function initFocusUnits() {
    if (!window.Rika6Report || !window.Rika6Report.getFocusUnits) return;
    window.Rika6Report.getFocusUnits().then(function (keys) {
      if (keys && keys.length) { focusUnits = new Set(keys); renderMenu(); }
    }).catch(function () { /* fail soft — all units stay visible */ });
  }

  // expose internals for tests
  window.__rika6Test = { state, UNIT_BY_KEY, buildQuestions, shuffle,
    setFocusUnits: function (keys) { focusUnits = keys ? new Set(keys) : null; renderMenu(); } };
