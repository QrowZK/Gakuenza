// sansu4-report.js — backend wiring for the 算数6年 module.
...
(function () {
  ...
  function getClient() {
    ...
    try {
      ...
      console.log('[Sansu6Report] supabase client unavailable:', e && e.message);
    }
    return client;
  }
  ...
  async function resolveContext() {
    ...
    if (!session || !session.user) {
      console.log('[Sansu6Report] no session — skipping reporting.');
      return null;
    }
    ...
    if (!mod || !schoolId) {
      console.log('[Sansu6Report] no module/school context — skipping reporting.');
  ...
  function getContext() {
    if (!ctxPromise) ctxPromise = resolveContext().catch(function (e) {
      console.log('[Sansu6Report] context resolution failed:', e && e.message);
      return null;
    });
    return ctxPromise;
  }
  ...
  async function report(result) {
    try {
      ...
      if (!window.HubCommon || !window.HubCommon.reportActivityWithItems) {
        console.log('[Sansu6Report] HubCommon.reportActivityWithItems unavailable — skipping.');
        return false;
      }
      ...
      // activity_ref, so 'sansu4/<sectionId>/<ts>' groups attempts of
      ...
      if (!out.ok) {
        console.log('[Sansu6Report] insert failed:', out.error && out.error.message);
        return false;
      }
      return true;
    } catch (e) {
      console.log('[Sansu6Report] report failed:', e && e.message);
      return false;
    }
  }
  ...
  async function getFocusUnits() {
    try {
      ...
      console.log('[Sansu6Report] focus-units resolution failed:', e && e.message);
      return null;
    }
  }
  ...
  window.Sansu6Report = { report, getProfile, getFocusUnits, signOut, MODULE_KEY };
})();
