// nhvocab-report.js — the module-contract adapter, same role as
// nh6-report.js / eiken-report.js.
// Replaces TangoApp's own login modal + independent Supabase backend (a
...
//
// Like nh6, app.js here calls window.hk.getUser() / window.hk.syncQuizResult()
...
(async function () {
  ...
  const [{ data: moduleRow }, { data: enrollmentRows }] = await Promise.all([
    sb.from('modules').select('id').eq('key', 'nhvocab').maybeSingle(),
  ...
  if (!moduleId || !schoolId) {
    console.warn('[NHVocabReport] no module/school context for this account — results will not be reported.');
  }

  // The shim itself. app.js's showResults() (unchanged from the original
  // app) calls exactly these two methods, guarded by
  // `typeof window.hk !== 'undefined' && state.qtype !== 'flash'`:
