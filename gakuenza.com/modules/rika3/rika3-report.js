...
(function () {
  ...
  // Unit-scoped pacing (focus_units) — same contract as sansu3 (the reference).
  // Returns the union of the focus-unit key lists set on this module across
  // every class the student is enrolled in, or null meaning "all units" (the
  // module then behaves exactly as before). Rule: if this module is assigned
  // to a class with focus_units = null ("all units for that class"), or isn't
  // assigned at all, return null and foreground nothing — we only foreground
  // when the scope is unambiguous. Fails soft: any error resolves to null.
  async function getFocusUnits() {
    try {
      ...
      console.log('[Rika3Report] focus-units resolution failed:', e && e.message);
      return null;
    }
  }
  ...
  window.Rika3Report = { report, getProfile, getFocusUnits, signOut, MODULE_KEY };
})();
