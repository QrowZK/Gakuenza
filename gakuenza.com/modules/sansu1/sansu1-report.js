// sansu1-report.js — backend wiring for the 算数1年 module.
...
(function () {
  ...
  function getClient() {
    ...
    try {
      ...
      console.log('[Sansu1Report] supabase client unavailable:', e && e.message);
    }
    return client;
  }
  ...
  async function resolveContext() {
    ...
    if (!session || !session.user) {
      console.log('[Sansu1Report] no session — skipping reporting.');
      return null;
    }
    ...
    if (!mod || !schoolId) {
      console.log('[Sansu1Report] no module/school context — skipping reporting.');
  ...
  function getContext() {
    if (!ctxPromise) ctxPromise = resolveContext().catch(function (e) {
      console.log('[Sansu1Report] context resolution failed:', e && e.message);
      return null;
    });
    return ctxPromise;
  }
  ...
  async function report(result) {
    try {
      ...
      if (!window.HubCommon || !window.HubCommon.reportActivityWithItems) {
        console.log('[Sansu1Report] HubCommon.reportActivityWithItems unavailable — skipping.');
        return false;
      }
      ...
      // activity_ref, so 'sansu1/<sectionId>/<ts>' groups attempts of the
      // same section into one gradebook column.
      ...
      if (!out.ok) {
        console.log('[Sansu1Report] insert failed:', out.error && out.error.message);
        return false;
      }
      return true;
    } catch (e) {
      console.log('[Sansu1Report] report failed:', e && e.message);
      return false;
    }
  }
  ...
  // null meaning "all units". Fails soft: any error resolves to null.
  async function getFocusUnits() {
    try {
      ...
      console.log('[Sansu1Report] focus-units resolution failed:', e && e.message);
      return null;
    }
  }
  ...
  window.Sansu1Report = { report, getProfile, getFocusUnits, signOut, MODULE_KEY };
})();
