// 算数5年 module — app logic (ported from sansu3/sansu4's generator-driven flow)
(function () {
  'use strict';
  ...
  // Canonical unit key is stored on each unit as u.key (u01_decimals …
  // u18_prisms_cylinders). These are the keys stored in
  // class_modules.focus_units and offered by the assignment UIs. null focus
  // = all units (today's behavior); a populated set foregrounds those units
  // without hiding others.
  function unitKey(u) { return u.key; }
  ...
  window.SANSU5_CHECK = { normalize: normalize, isCorrect: isCorrect };
  ...
  function renderMenu() {
    ...
    UNITS.forEach(function (u) {
      ...
      if (u.vol !== currentVol) {
        ...
        h.textContent = currentVol === '上' ? '5年 上' : '5年 下';
        root.appendChild(h);
      }
  ...
  function startSection(unit, s) {
    ...
    while (problems.length < s.n && guard++ < s.n * 30) {
      ...
      // prompt AND answer (figure-based problems can share prompt text but
  ...
  function finish() {
    ...
    if (window.Sansu5Report) {
      window.Sansu5Report.report({
  ...
  async function initAccount() {
    if (!window.Sansu5Report) return;
    try {
      const profile = await window.Sansu5Report.getProfile();
      if (!profile) return;
      ...
      $('signoutBtn').addEventListener('click', async function () {
        await window.Sansu5Report.signOut();
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (window.Sansu5Report && window.Sansu5Report.getFocusUnits) {
      window.Sansu5Report.getFocusUnits().then(function (keys) {
