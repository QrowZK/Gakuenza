// 社会4年 module — app logic.
// Adapted from the 社会3年 engine. Differences worth knowing:
//   - honors class_modules.focus_units: assigned units are foregrounded at
//     the top of もくじ; the rest stay visible (dimmed) below a divider —
//     never hard-hidden. Fails soft to "all units" on null/empty/no-match.
//   - reporting sends per-question `items` so activity_result_items gets
//     populated (via HubCommon.reportActivityWithItems in the report shim).
(function () {
  'use strict';

  const { UNITS, FIGURES } = window.SHAKAI4_DATA;
  ...
  const state = {
    ...
    focusUnits: null,    // array of unit keys (class_modules.focus_units) or null
  };
  ...
  async function refreshFukushuCard() {
    ...
    if (state.backendRows === null && window.Shakai4Report) {
      meta.textContent = '読み込み中…';
      try {
        state.backendRows = await window.Shakai4Report.fetchResults();
      } catch (e) {
        state.backendRows = [];
      }
  ...
  // ---------- focus_units ordering ----------
  // Foreground the assigned units; keep the rest visible (dimmed) below.
  // Fail soft to "all units, normal order" when focus_units is null, empty,
  // malformed, or matches nothing (so a stale/typo'd key never hides the
  // whole module).
  function partitionUnits() {
    const f = state.focusUnits;
    if (!Array.isArray(f) || !f.length) return { focus: UNITS, rest: [], scoped: false };
    const set = new Set(f);
    const focus = UNITS.filter((u) => set.has(u.key));
    const rest = UNITS.filter((u) => !set.has(u.key));
    if (!focus.length) return { focus: UNITS, rest: [], scoped: false };
    return { focus, rest, scoped: true };
  }

  function unitBlock(unit, dimmed) {
    const block = document.createElement('div');
    block.className = 'unit-block ' + unit.color + (dimmed ? ' dimmed' : '');
  ...
  function renderMenu() {
    ...
    const { focus, rest, scoped } = partitionUnits();
    ...
    focus.forEach((unit) => root.appendChild(unitBlock(unit, false)));
    ...
    if (rest.length) {
      ...
      rest.forEach((unit) => root.appendChild(unitBlock(unit, true)));
    }
  ...
  async function initAccountBubble() {
    try {
      if (!window.Shakai4Report) return;
      const profile = await window.Shakai4Report.getProfile();
      if (!profile) return;
      const name =
      ...
      $('signoutBtn').addEventListener('click', async () => {
        await window.Shakai4Report.signOut();
        location.href = '../../hub/login.html';
      });
    } catch (e) {
      console.log('[Shakai4] account bubble skipped:', e && e.message);
    }
  }
  ...
  async function initFocusUnits() {
    try {
      if (!window.Shakai4Report || !window.Shakai4Report.getFocusUnits) return;
      const fu = await window.Shakai4Report.getFocusUnits();
      ...
      console.log('[Shakai4] focus_units skipped:', e && e.message);
    }
  }

  // expose internals for tests
  window.__shakai4Test = { normalize, isCorrect, computeWrongIds, partitionUnits, QUESTION_INDEX, state, UNITS };
