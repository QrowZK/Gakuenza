// rika5-report.js — backend wiring for the 理科5年 module.
...
//
// rika5 also honors class_modules.focus_units via getFocusUnits(), matching
...
(function () {
  ...
  function getClient() {
    ...
    try {
      ...
      console.log('[Rika5Report] supabase client unavailable:', e && e.message);
    }
    return client;
  }
  ...
  async function resolveContext() {
    ...
    if (!session || !session.user) {
      console.log('[Rika5Report] no session — skipping reporting.');
      return null;
    }
    ...
    if (!mod || !schoolId) {
      console.log('[Rika5Report] no module/school context — skipping reporting.');
  ...
  function getContext() {
    if (!ctxPromise) ctxPromise = resolveContext().catch(function (e) {
      console.log('[Rika5Report] context resolution failed:', e && e.message);
      return null;
    });
    return ctxPromise;
  }
  ...
  async function report(result) {
    try {
      ...
      if (!window.HubCommon || !window.HubCommon.reportActivityWithItems) {
        console.log('[Rika5Report] HubCommon.reportActivityWithItems unavailable — skipping.');
        return false;
      }
      ...
      // activity_ref, so 'rika5/<unitKey>/<ts>' groups every attempt of the
      ...
      if (!out.ok) {
        console.log('[Rika5Report] insert failed:', out.error && out.error.message);
        return false;
      }
      return true;
    } catch (e) {
      console.log('[Rika5Report] report failed:', e && e.message);
      return false;
    }
  }
  ...
  // error resolves to null (no regression). Keys are rika5's unit keys
  // (u01_weather..u10_pendulum); see hub/module-units.js.
  async function getFocusUnits() {
    try {
      ...
      console.log('[Rika5Report] focus-units resolution failed:', e && e.message);
      return null;
    }
  }
  ...
  window.Rika5Report = { report, getProfile, getFocusUnits, signOut, MODULE_KEY };
})();
