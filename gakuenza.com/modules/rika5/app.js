...
(function () {
  ...
  const { STRANDS } = window.RIKA5_DATA;
  ...
  function finishDrill() {
    ...
    if (window.Rika5Report) {
      window.Rika5Report.report({
        unitKey: u.key,
  ...
  async function initAccount() {
    if (!window.Rika5Report) return;
    try {
      const profile = await window.Rika5Report.getProfile();
      if (!profile) return;
      ...
      $('signoutBtn').addEventListener('click', async () => {
        await window.Rika5Report.signOut();
        window.location.href = '../../hub/login.html';
      });
    } catch (e) {
      console.log('[Rika5] account bubble skipped:', e && e.message);
    }
  }
  ...
  function initFocusUnits() {
    if (!window.Rika5Report || !window.Rika5Report.getFocusUnits) return;
    window.Rika5Report.getFocusUnits().then(function (keys) {
      if (keys && keys.length) { focusUnits = new Set(keys); renderMenu(); }
    }).catch(function () { /* fail soft — all units stay visible */ });
  }

  // expose internals for tests
  window.__rika5Test = { state, UNIT_BY_KEY, buildQuestions, shuffle,
