// sansu2-report.js — backend wiring for the 算数2年 module.
...
// (hub-common.js), per the established convention (hard rule 2).
// Context is resolved via enrollments → classes.school_id, never
// profiles.home_school_id (hard rule 3).
// If any resolution step fails, reporting silently skips — a practice
// attempt must never error out of the child's flow.
(function () {
  'use strict';

  const MODULE_KEY = 'sansu2';
  let client = null;
  let ctxPromise = null;
  ...
  function getClient() {
    ...
    try {
      ...
      console.log('[Sansu2Report] supabase client unavailable:', e && e.message);
    }
    return client;
  }
  ...
  async function resolveContext() {
    ...
    if (!session || !session.user) {
      console.log('[Sansu2Report] no session — skipping reporting.');
      return null;
    }
    ...
    if (!mod || !schoolId) {
      console.log('[Sansu2Report] no module/school context — skipping reporting.');
  ...
  function getContext() {
    if (!ctxPromise) ctxPromise = resolveContext().catch(function (e) {
      console.log('[Sansu2Report] context resolution failed:', e && e.message);
      return null;
    });
    return ctxPromise;
  }
  ...
  async function report(result) {
    try {
      ...
      if (!window.HubCommon || !window.HubCommon.reportActivityWithItems) {
        console.log('[Sansu2Report] HubCommon.reportActivityWithItems unavailable — skipping.');
        return false;
      }
      ...
      // activity_ref, so 'sansu2/<sectionId>/<ts>' groups attempts of
      ...
      if (!out.ok) {
        console.log('[Sansu2Report] insert failed:', out.error && out.error.message);
        return false;
      }
      return true;
    } catch (e) {
      console.log('[Sansu2Report] report failed:', e && e.message);
      return false;
    }
  }
  ...
  async function getFocusUnits() {
    try {
      ...
      console.log('[Sansu2Report] focus-units resolution failed:', e && e.message);
      return null;
    }
  }
  ...
  window.Sansu2Report = { report, getProfile, getFocusUnits, signOut, MODULE_KEY };
})();
