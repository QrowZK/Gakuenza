...
(function () {
  ...
  const { UNITS, FIGURES } = window.SHAKAI5_DATA;
  ...
  async function refreshFukushuCard() {
    ...
    if (state.backendRows === null && window.Shakai5Report) {
      meta.textContent = '読み込み中…';
      try {
        state.backendRows = await window.Shakai5Report.fetchResults();
      } catch (e) {
        state.backendRows = [];
      }
  ...
  async function initAccountBubble() {
    try {
      if (!window.Shakai5Report) return;
      const profile = await window.Shakai5Report.getProfile();
      if (!profile) return;
      const name =
      ...
      $('signoutBtn').addEventListener('click', async () => {
        await window.Shakai5Report.signOut();
        location.href = '../../hub/login.html';
      });
    } catch (e) {
      console.log('[Shakai5] account bubble skipped:', e && e.message);
    }
  }
  ...
  async function initFocusUnits() {
    try {
      if (!window.Shakai5Report || !window.Shakai5Report.getFocusUnits) return;
      const fu = await window.Shakai5Report.getFocusUnits();
      ...
      console.log('[Shakai5] focus_units skipped:', e && e.message);
    }
  }

  // expose internals for tests
  window.__shakai5Test = { normalize, isCorrect, computeWrongIds, partitionUnits, QUESTION_INDEX, state, UNITS };
