// shakai5-report.js — backend wiring for the 社会5年 module.
// Contract mirrors shakai-report.js (社会3年):
//   session -> userId -> profile
//   modules.key = 'shakai5' -> moduleId
...
(function () {
  ...
  function getClient() {
    ...
    try {
      ...
      console.log('[Shakai5Report] supabase client unavailable:', e && e.message);
    }
    return client;
  }
  ...
  async function resolveContext() {
    ...
    if (!session || !session.user) {
      console.log('[Shakai5Report] no session — skipping reporting.');
      return null;
    }
    ...
    if (!mod || !schoolId) {
      console.log('[Shakai5Report] no module/school context — skipping reporting.');
  ...
  function getContext() {
    if (!ctxPromise) ctxPromise = resolveContext().catch((e) => {
      console.log('[Shakai5Report] context resolution failed:', e && e.message);
      return null;
    });
    return ctxPromise;
  }
  ...
  async function report(result) {
    try {
      ...
      const activityRef = `shakai5/${result.sectionId}/${Date.now()}`;
      ...
      if (!res || !res.ok) {
        console.log('[Shakai5Report] report failed:', res && res.error && res.error.message);
        return false;
      }
      return true;
    } catch (e) {
      console.log('[Shakai5Report] report failed:', e && e.message);
      return false;
    }
  }
  ...
  async function fetchResults() {
    try {
      ...
      if (error) {
        console.log('[Shakai5Report] fetchResults failed:', error.message);
        return [];
      }
      return data || [];
    } catch (e) {
      console.log('[Shakai5Report] fetchResults error:', e && e.message);
      return [];
    }
  }
  ...
  async function getFocusUnits() {
    try {
      ...
      const { data, error } = await sb
        ...
        .maybeSingle();
      if (error) {
        console.log('[Shakai5Report] getFocusUnits failed:', error.message);
        return null;
      }
      ...
      console.log('[Shakai5Report] getFocusUnits error:', e && e.message);
      return null;
    }
  }
  ...
  window.Shakai5Report = { report, fetchResults, getFocusUnits, getProfile, signOut, MODULE_KEY };
})();
