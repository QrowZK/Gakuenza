...
// KANJI4 (kanji-data.js). Same "generator, not fixed bank" pattern as kokugo3/5's
// kanji drills: 202 real grade-4 kanji × multiple question types × randomized
...
// construction and asserted in kanji-generator.test.js at scale.
...
function genReadingQuestion(KANJI4) {
  const idx = Math.floor(Math.random() * KANJI4.length);
  const target = KANJI4[idx];
  ...
  const distractorIdxs = pickRandom(KANJI4, 8, idx);
  ...
  return {
    itemRef: `kanji4/reading/${target.k}/${correctClean}`,
    category: '漢字：読み方',
...
function genKanjiSelectQuestion(KANJI4) {
  const idx = Math.floor(Math.random() * KANJI4.length);
  const target = KANJI4[idx];
  const allReadings = [...target.on, ...target.kun];
  ...
  // the target itself. Grade-4 is dense with homophones (械・害・街 all カイ;
  // 官・完・管・関・観 all カン; 径・景・芸 all ケイ), so a distractor sharing the
  // correct reading makes the question genuinely ambiguous, not merely wrong.
  // Checked against each distractor's own readings, not assumed safe from
  // excluding the index.
  const candidates = KANJI4.filter((k, i) =>
  ...
  return {
    itemRef: `kanji4/select/${target.k}/${correctReadingClean}`,
    category: '漢字：漢字えらび',
...
function genStrokeCountQuestion(KANJI4) {
  const idx = Math.floor(Math.random() * KANJI4.length);
  const target = KANJI4[idx];
  const correct = target.strokes;

  // Draw distractors from the FULL 202-kanji stroke-count pool, not a small
  ...
  const allOtherStrokes = [...new Set(KANJI4.map(k => k.strokes).filter(s => s !== correct))];
  const distractors = shuffle(allOtherStrokes).slice(0, 3);
  const options = shuffle([correct, ...distractors]).map(String);

  return {
    itemRef: `kanji4/strokes/${target.k}/${correct}`,
    category: '漢字：画数',
...
function generateKanjiQuiz(KANJI4, count) {
  const questions = [];
  for (let i = 0; i < count; i++) {
    const gen = GENERATORS[Math.floor(Math.random() * GENERATORS.length)];
    questions.push(gen(KANJI4));
  }
  return questions;
}
