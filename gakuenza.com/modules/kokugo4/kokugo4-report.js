// kokugo4-report.js — reports a completed drill (kanji quiz or grammar unit)
// via the shared HubCommon.reportActivityWithItems helper, same pattern as
// kokugo3/5's report shims. Deliberately does NOT hand-roll the activity_results/
...
// using the shared helper. kokugo4 has no reading-comprehension mode yet, so
// only 'kanji' and 'grammar' are handled.
(function () {
  async function reportKokugo4(sb, ctx, { mode, unitKey, results }) {
    // mode: 'kanji' | 'grammar'
    ...
    let activityRef;
    if (mode === 'kanji') activityRef = `kokugo4/kanji/${Date.now()}`;
    else activityRef = `kokugo4/grammar/${unitKey}/${Date.now()}`;
  ...
  window.Kokugo4Report = { reportKokugo4 };
})();
