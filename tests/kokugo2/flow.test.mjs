// flow.test.mjs — headless end-to-end flow test for kokugo2.
...
// so no real backend is needed, drives each mode (kanji + the five ことば
...
window.supabase = { createClient: function () {
  function builder(table) {
    var b = {
      ...
      _result: function(){
        if (b._table === 'modules') return { data: { id: 'mod-kokugo2' }, error: null };
...
async function runMode(page, label, openSteps, expect) {
  ...
  await page.goto(BASE + '/modules/kokugo2/index.html', { waitUntil: 'networkidle' });
  ...
  while (answered < 40) {
    ...
    if (done) break;
    await page.waitForSelector('.k2-opt:not(:disabled)');
    await page.click('.k2-opt:not(:disabled)');
    answered++;
    await page.waitForFunction(() =>
      document.getElementById('view-result').style.display === 'block' ||
      document.querySelectorAll('.k2-opt:not(:disabled)').length > 0
    );
  }
  ...
  const r = reports[0];
  if (r.moduleId !== 'mod-kokugo2') fail(`${label}: wrong moduleId ${r.moduleId}`);
...
try {
  await runMode(page, 'kanji',
    ...
    r => r.activityRef.startsWith('kokugo2/kanji/') && r.payload.mode === 'kanji');

  for (const unit of ['katakana', 'kanazukai', 'shugo_jutsugo', 'nakama', 'kutouten']) {
    await runMode(page, `grammar:${unit}`,
      async p => {
        await p.click('#btn-mode-grammar');
        await p.click(`[data-gunit="${unit}"]`);
      },
      r => r.activityRef.startsWith(`kokugo2/grammar/${unit}/`) && r.payload.mode === 'grammar' && r.payload.unit === unit);
  }
} finally {
  await browser.close();
  server.close();
