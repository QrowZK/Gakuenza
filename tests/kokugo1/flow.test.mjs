// flow.test.mjs — headless end-to-end flow test for kokugo1.
...
// so no real backend is needed, drives each of the five units (ひらがな /
// カタカナ / は・を・へ / 句読点 / かんじ) through the actual quiz UI to the
// result screen, and asserts that the shared reporting helper
...
window.supabase = { createClient: function () {
  function builder(table) {
    var b = {
      ...
      _result: function(){
        if (b._table === 'modules') return { data: { id: 'mod-kokugo1' }, error: null };
...
async function runMode(page, label, unitKey, expect) {
  await page.evaluate(() => { window.__reports = []; });
  await page.goto(BASE + '/modules/kokugo1/index.html', { waitUntil: 'networkidle' });
  await page.waitForSelector(`[data-unit="${unitKey}"]`);
  await page.click(`[data-unit="${unitKey}"]`);
  let answered = 0;
  while (answered < 40) {
    const done = await page.evaluate(() => document.getElementById('view-result').style.display === 'block');
    if (done) break;
    await page.waitForSelector('.k1-opt:not(:disabled)');
    await page.click('.k1-opt:not(:disabled)');
    answered++;
    await page.waitForFunction(() =>
      document.getElementById('view-result').style.display === 'block' ||
      document.querySelectorAll('.k1-opt:not(:disabled)').length > 0
    );
  }
  ...
  const r = reports[0];
  if (r.moduleId !== 'mod-kokugo1') fail(`${label}: wrong moduleId ${r.moduleId}`);
...
try {
  for (const unit of ['hiragana', 'katakana', 'joshi', 'kutouten', 'kanji']) {
    await runMode(page, unit, unit,
      r => r.activityRef.startsWith(`kokugo1/${unit}/`) && r.payload.unit === unit);
  }
} finally {
  await browser.close();
  server.close();
