// flow.test.mjs — headless end-to-end flow test for sansu5.
// Run: node flow.test.mjs   (requires playwright + chromium)
//
// Serves the site over HTTP, stubs the hub scripts (supabase/config/hub-common)
// so no real backend is needed, drives a section through the actual quiz UI
...
//
// It exercises BOTH answer kinds: a typed-heavy unit and a choice-heavy unit,
// answering every question with the *correct* value (read out of the live
// problem object via window.SANSU5_DATA) so we also confirm the report's score
// equals maxScore end-to-end.
...
window.supabase = { createClient: function () {
  function builder(table) {
    var b = {
      ...
      _result: function(){
        if (b._table === 'modules') return { data: { id: 'mod-sansu5' }, error: null };
...
async function runSection(page, label, unitNum, sectionId, expectN) {
  ...
  await page.goto(BASE + '/modules/sansu5/index.html', { waitUntil: 'networkidle' });
  ...
  // (e.g. graph "which is largest?"), which a text-match probe gets wrong.
  ...
  const r = reports[0];
  if (r.moduleId !== 'mod-sansu5') fail(`${label}: wrong moduleId ${r.moduleId}`);
  ...
  if (!r.activityRef.startsWith('sansu5/' + sectionId + '/')) fail(`${label}: activityRef ${r.activityRef}`);
...
// every produced problem in a queue, then map the visible question text back to
// its problem object. Runs via addInitScript (re-applied on every navigation),
// polling until SANSU5_DATA exists so the wrap lands after the module loads but
// before any section starts.
await page.addInitScript(() => {
  const iv = setInterval(() => {
    if (window.SANSU5_DATA && !window.__probeInstalled) {
...
try {
  // u09 fraction/decimal — mixes typed + choice; u08 multiples/factors — has choice;
  // u13 area — all typed; u18 prisms — has choice. Cover a representative spread.
  await runSection(page, 'u09_fraction_decimal', 9, 'u09-fd', 10);
  await runSection(page, 'u13_area', 13, 'u13-area', 10);
  await runSection(page, 'u18_prisms_cylinders', 18, 'u18-prism', 8);
  await runSection(page, 'u15_graphs', 15, 'u15-graph', 8);
  await runSection(page, 'u01_decimals', 1, 'u01-dec', 10);
} finally {
  await browser.close();
  server.close();
}
