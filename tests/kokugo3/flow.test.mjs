// flow.test.mjs — headless end-to-end flow test for kokugo3.
...
// so no real backend is needed, drives each mode (kanji / reading / grammar)
// through the actual quiz UI to the result screen, and asserts that the shared
...
const SITE_ROOT = path.resolve(__dirname, '../../gakuenza.com'); // gakuenza.com/
...
window.supabase = { createClient: function () {
  function builder(table) {
    var b = {
      ...
      _result: function(){
        if (b._table === 'modules') return { data: { id: 'mod-kokugo3' }, error: null };
...
async function runMode(page, label, openSteps, expect) {
  ...
  await page.goto(BASE + '/modules/kokugo3/index.html', { waitUntil: 'networkidle' });
  ...
  while (answered < 40) {
    ...
    if (done) break;
    await page.waitForSelector('.k3-opt:not(:disabled)');
    await page.click('.k3-opt:not(:disabled)');
    answered++;
    // page disables options then auto-advances after ~700ms; wait for either the
    // next enabled option or the result screen.
    await page.waitForFunction(() =>
      document.getElementById('view-result').style.display === 'block' ||
      document.querySelectorAll('.k3-opt:not(:disabled)').length > 0
    );
  }
  ...
  const r = reports[0];
  if (r.moduleId !== 'mod-kokugo3') fail(`${label}: wrong moduleId ${r.moduleId}`);
...
try {
  await runMode(page, 'kanji',
    ...
    r => r.activityRef.startsWith('kokugo3/kanji/') && r.payload.mode === 'kanji');

  await runMode(page, 'reading:mochimochi',
    async p => {
      await p.click('#btn-mode-reading');
      await p.click('[data-unit="mochimochi"]');
    },
    r => r.activityRef.startsWith('kokugo3/reading/mochimochi/') && r.payload.unit === 'mochimochi');

  await runMode(page, 'grammar:romaji',
    async p => {
      await p.click('#btn-mode-grammar');
      await p.click('[data-gunit="romaji"]');
    },
    r => r.activityRef.startsWith('kokugo3/grammar/romaji/') && r.payload.mode === 'grammar' && r.payload.unit === 'romaji');

  await runMode(page, 'grammar:kotowaza',
    async p => {
      await p.click('#btn-mode-grammar');
      await p.click('[data-gunit="kotowaza"]');
    },
    r => r.activityRef.startsWith('kokugo3/grammar/kotowaza/') && r.payload.mode === 'grammar');

  await runMode(page, 'grammar:kosoado',
    async p => {
      await p.click('#btn-mode-grammar');
      await p.click('[data-gunit="kosoado"]');
    },
    r => r.activityRef.startsWith('kokugo3/grammar/kosoado/') && r.payload.mode === 'grammar');

  await runMode(page, 'grammar:shuushoku',
    async p => {
      await p.click('#btn-mode-grammar');
      await p.click('[data-gunit="shuushoku"]');
    },
    r => r.activityRef.startsWith('kokugo3/grammar/shuushoku/') && r.payload.mode === 'grammar');
} finally {
  await browser.close();
  server.close();
}
