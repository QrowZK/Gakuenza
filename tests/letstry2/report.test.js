// report.test.js — report-path integration test for letstry2 (no browser needed).
// Run: node tests/letstry2/report.test.js
//
// Loads the REAL hub-common.js and letstry2-report.js under a minimal
// window/DOM shim with a stubbed Supabase client, then calls
// window.hk.syncQuizResult(...) (the shim's one method — index.html's
// showReview() calls exactly this, guarded by
// `typeof window.hk !== 'undefined'`) with a realistic quiz-shaped payload
// including items[], and asserts:
...
//     gap that letstry2 itself used to have (#126): this shim used to
//     hand-roll the activity_results insert and never wrote item detail.
//   - item rows carry item_ref / prompt / category / correct /
//     selected_answer / correct_answer
//   - activity_ref is shaped like "letstry2/<category>/<level>/<ts>"
// Also covers the failure path: letstry2's shim deliberately re-throws when
// the shared helper reports ok:false (index.html's try/catch shows the
// "⚠️ 保存に失敗しました" banner), so a second call with a failing
// activity_results insert must reject rather than resolve silently.
const fs = require('fs');
...
// Toggled mid-test to exercise the failure path without a second module load.
const state = { failActivityResults: false };
...
function makeClient() {
  function builder(table) {
    const b = {
      ...
      then(resolve) {
        queried.push(table);
        if (table === 'modules') return resolve({ data: { id: 'mod-letstry2' }, error: null });
        ...
        if (table === 'activity_results') {
          if (state.failActivityResults) return resolve({ data: null, error: new Error('insert failed (stub)') });
          return resolve({ data: { id: 'ar-1' }, error: null });
        }
...
// ── Window / DOM shim ───────────────────────────────────────────────────────
// letstry2-report.js renders an account bubble into #module-account-mount
...
eval(fs.readFileSync(path.join(base, 'modules/letstry2/letstry2-report.js'), 'utf8'));

(async () => {
  // letstry2-report.js is an async IIFE — give its internal awaits
  ...
  // Quiz-shaped payload matching index.html's showReview() call:
  //   window.hk.syncQuizResult({ level, setId, category, correct, total, app_id, items })
  // items come from qResults (both quiz and build activities report items).
  const items = [
  ...
  if (ar.module_id !== 'mod-letstry2') fail(`wrong module_id ${ar.module_id}`);
  ...
  // letstry2-report.js builds activityRef as `letstry2/${category}/${level}/${Date.now()}`.
  if (!String(ar.activity_ref).startsWith('letstry2/quiz/unit0/')) fail(`activity_ref ${ar.activity_ref}`);
  ...
  // ── Failure path: the shim deliberately re-throws when the helper fails ──
  state.failActivityResults = true;
  let threw = false;
  try {
    await window.hk.syncQuizResult({
      app_id: 'letstry2',
  ...
  if (!threw) fail('syncQuizResult did not throw when reportActivityWithItems reported ok:false');

  console.log('letstry2 report-path integration test');
  ...
  console.log(`  failure-path rejects:  ${threw}`);
  if (errors.length) {
