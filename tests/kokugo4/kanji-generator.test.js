// Stress test for kokugo4/kanji-generator.js. Run: node kanji-generator.test.js
...
const KANJI4 = require('../../gakuenza.com/modules/kokugo4/kanji-data.js');
const {
  generateKanjiQuiz, genReadingQuestion, genKanjiSelectQuestion, genStrokeCountQuestion,
} = require('../../gakuenza.com/modules/kokugo4/kanji-generator.js');

let failures = 0;
const fail = (msg) => { failures++; if (failures <= 40) console.error('FAIL:', msg); };

// ── data integrity ──
if (KANJI4.length !== 202) fail(`expected 202 grade-4 kanji, got ${KANJI4.length}`);
{
  const seen = new Set();
  KANJI4.forEach(k => {
    ...
    // no bound-form-marker readings should have survived data cleaning
    [...k.on, ...k.kun].forEach(r => { if (/^-|-$/.test(r)) fail(`${k.k}: bound-form marker leaked: ${r}`); });
  });
  const distinctStrokes = new Set(KANJI4.map(k => k.strokes));
...
for (let i = 0; i < N; i++) {
  ...
  // ── select question ──
  {
    const q = genKanjiSelectQuestion(KANJI4);
...
{
  const quiz = generateKanjiQuiz(KANJI4, 10);
  if (quiz.length !== 10) fail(`generateKanjiQuiz(10) length ${quiz.length}`);
  quiz.forEach(q => checkStructural('quiz', q));
}

if (failures === 0) {
  console.log(`OK — ${KANJI4.length} kanji, 3 generators x ${N} each, all structural + collision checks passed.`);
} else {
  console.error(`\n${failures} failure(s).`);
  process.exit(1);
}
