// flow.test.mjs — headless end-to-end flow test for kokugo4.
...
// so no real backend is needed, drives each mode (kanji + the four grammar
// units) through the actual quiz UI to the result screen, and asserts that the
// shared reporting helper HubCommon.reportActivityWithItems was called with the
// right module_id AND a populated per-question `items` array
// (activity_result_items) — the exact gap the shared-infra doc warns modules
// keep shipping.
...
window.supabase = { createClient: function () {
  function builder(table) {
    var b = {
      ...
      _result: function(){
        if (b._table === 'modules') return { data: { id: 'mod-kokugo4' }, error: null };
...
async function runMode(page, label, openSteps, expect) {
  ...
  await page.goto(BASE + '/modules/kokugo4/index.html', { waitUntil: 'networkidle' });
  ...
  while (answered < 40) {
    ...
    if (done) break;
    await page.waitForSelector('.k4-opt:not(:disabled)');
    await page.click('.k4-opt:not(:disabled)');
    answered++;
    await page.waitForFunction(() =>
      document.getElementById('view-result').style.display === 'block' ||
      document.querySelectorAll('.k4-opt:not(:disabled)').length > 0
    );
  }
  ...
  const r = reports[0];
  if (r.moduleId !== 'mod-kokugo4') fail(`${label}: wrong moduleId ${r.moduleId}`);
...
try {
  await runMode(page, 'kanji',
    ...
    r => r.activityRef.startsWith('kokugo4/kanji/') && r.payload.mode === 'kanji');

  for (const unit of ['bushu', 'jukugo', 'setsuzoku', 'shugo_jutsugo', 'kanyouku']) {
    await runMode(page, `grammar:${unit}`,
      async p => {
        await p.click('#btn-mode-grammar');
        await p.click(`[data-gunit="${unit}"]`);
      },
      r => r.activityRef.startsWith(`kokugo4/grammar/${unit}/`) && r.payload.mode === 'grammar' && r.payload.unit === unit);
  }
} finally {
  await browser.close();
  server.close();
