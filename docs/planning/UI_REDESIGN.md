...
| **A — Redesigned 2026-07-24** | `letstry1`, `letstry2` | inline (still no `style.css`) | — | **Re-skinned to the shared washi/indigo design system from a design mockup (#165 / #164).** Unit-card grid with per-unit accents + progress bars, cream mode-picker, quiz cards, conic-gradient results donut; Zen Maru Gothic display, dark mode preserved, self-contained CSS. Was previously "out of visual scope (inline CSS, ported apps)". With `eigo5`/`eigo6` (redesigned earlier), the whole **外国語 family (grades 3–6) now shares one visual system.** Deeper polish of the activity internals remains backlogged in design. |
